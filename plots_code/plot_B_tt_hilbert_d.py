...
data_file_name = "B_tt_hilbert_d.csv"
cols = ["d", "algorithm", "t1", "t2", "time_taken", "error"]

df = pd.read_csv(data_path_prefix + data_file_name, usecols=cols).dropna()

file_names = ["B_tt_hilbert_tt_svd_round_ratio_d", "B_tt_hilbert_tt_rsvd_round_ratio_d", "B_tt_hilbert_tt_rand_orth_round_ratio_d"]
...
for alg_id, alg in enumerate(algorithms):  
    ...
    percentages = [[t1[i]/time_taken[alg_id][i], t2[i]/time_taken[alg_id][i], (time_taken[alg_id][i]-t1[i]-t2[i])/time_taken[alg_id][i]] for i in range(len(d))]
    ...
    ax.set_title('Rounding of Hilbert tensor train ($n=5$, $r=10$) with ' + alg)
...
file_name = "B_tt_hilbert_time_d"

plt.plot(d, time_taken[0], "-", label=labels[0], ms=3)
plt.plot(d, time_taken[1], "-", label=labels[1], ms=3)
plt.plot(d, time_taken[2], "-", label=labels[2], ms=3)

plt.xticks(d)
plt.ylabel("Runtime")
plt.xlabel("Number of dimensions ($d$)")
plt.legend()
plt.title("Rounding of Hilbert tensor train ($n=5$, $r=10$)")
...
labels = ["tt_svd_rounding", "tt_rsvd_rounding", "tt_rand_orth_rounding"]
file_name = "B_tt_hilbert_error_d"
