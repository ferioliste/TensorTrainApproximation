...
data_file_name = "B_tt_random_sum_n.csv"
cols = ["n", "algorithm", "t1", "t2", "time_taken", "error"]

df = pd.read_csv(data_path_prefix + data_file_name, usecols=cols).dropna()

file_names = ["B_tt_random_sum_tt_svd_round_ratio_n", "B_tt_random_sum_tt_rsvd_round_ratio_n", "B_tt_random_sum_tt_rand_orth_round_ratio_n"]
...
for alg_id, alg in enumerate(algorithms):  
    ...
    ax.set_title('Rounding of random-sum tensor train ($d=5$, $r=10$, $\\varepsilon=10^{-8}$) with ' + alg)
...
file_name = "B_tt_random_sum_time_n"
...
plt.legend()
plt.title("Rounding of random-sum tensor train ($d=5$, $r=10$, $\\varepsilon=10^{-8}$)")
...
labels = ["tt_svd_rounding", "tt_rsvd_rounding", "tt_rand_orth_rounding"]
file_name = "B_tt_random_sum_error_n"
