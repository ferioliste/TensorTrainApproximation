...
data_file_name = "B_tt_random_sum_p.csv"
cols = ["pert", "algorithm", "t1", "t2", "time_taken", "error"]

df = pd.read_csv(data_path_prefix + data_file_name, usecols=cols).dropna()

file_names = ["B_tt_random_sum_tt_svd_round_ratio_p", "B_tt_random_sum_tt_rsvd_round_ratio_p", "B_tt_random_sum_tt_rand_orth_round_ratio_p"]
...
for alg_id, alg in enumerate(algorithms):  
    alg_df = df[df['algorithm'] == alg].sort_values(by="pert").drop(columns=['algorithm']).groupby("pert")
    p  = list(alg_df.groups.keys())
    ...
    percentages = [[t1[i]/time_taken[alg_id][i], t2[i]/time_taken[alg_id][i], (time_taken[alg_id][i]-t1[i]-t2[i])/time_taken[alg_id][i]] for i in range(len(p))]
    ...
    ax.set_title('Rounding of random-sum tensor train ($d=5$, $n=5$, $r=20$) with ' + alg)
    ax.set_xticks([0, 3, 7, 11, 15])
    ax.set_xticklabels(["$10^{-15}$", "$10^{-11}$", "$10^{-7}$", "$10^{-4}$", "$1$"])
    ax.legend()
...
file_name = "B_tt_random_sum_time_p"

plt.plot(p, time_taken[0], "-", label=labels[0], ms=3)
plt.plot(p, time_taken[1], "-", label=labels[1], ms=3)
plt.plot(p, time_taken[2], "-", label=labels[2], ms=3)

plt.xticks(p)
plt.ylabel("Runtime")
plt.xlabel("Perturbation ($\\varepsilon$)")
plt.legend()
plt.xscale("log")
plt.title("Rounding of random-sum tensor train ($d=5$, $n=5$, $r=20$)")
...
labels = ["tt_svd_rounding", "tt_rsvd_rounding", "tt_rand_orth_rounding"]
file_name = "B_tt_random_sum_error_p"

offset = 10.**0.1

plt.plot(p, errors05[0], "-o", label=labels[0], ms=3)
