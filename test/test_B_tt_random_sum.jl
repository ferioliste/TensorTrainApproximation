function run_test(file_path::String, row_index::Int; save::Bool = true)
    parameters_names = ["tensor", "d", "n", "rank", "pert", "algorithm"];
    parameters = get_test_parameters(file_path, row_index, parameters_names);

    seed::Int64 = rand(UInt32)

    TT = test_tensor(parameters["tensor"], parameters["n"], parameters["d"], pert=parameters["pert"], seed=seed+1)
    Tfull = tt2full(TT)
...
file_path = "../test_results/B_tt_random_sum_" * ending * ".csv"
